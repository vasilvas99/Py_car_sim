...
class SimulatedCar:
    """
    A simple (black-box) car kinematics simulator. It takes as input: gas position (0, 1), brake position (0,1) and steering angle (-pi/2, pi/2).
    It is assumed that breaking and accelerating are two antagonistic effects that can't happend at the same time and that they are related linearly
    to the acceleration (this models the "jerk" when driving a car).
    This assumption is then used in combination with the bicycle vehicle model (see: https://thef1clan.com/2020/09/21/vehicle-dynamics-the-kinematic-bicycle-model/)
    to propage the simulated car in 2D space. After each propagation step velocity and acceleration componets are calculated.
    """
    ...
    def __init__(
        ...
        # these two are needed in order to calculate the components of the acceleration at the end of the update cycle
        self._old_v_x = self._v_x
        self._old_v_y = self._v_y

    # Setup proper r/w access to the interal state, returning vector quantities as (x,y) tuples and scalars as a single value
    @property
    ...
    def acc_from_ctrl(self):
        """Gas and brake are combined in a single variable "control position" that can take any value in the interval [-1,1].
        An implicit assumption here is that you cannot press the gas and the brake at the same time.
        Where ctrl_position  > 0 implies acceleration and ctrl_position < 0 implies brakeing. A pieciewise linear relation betwen
        the ctrl_position and the "signed norm" of the acceleration (> 0 - acceleration, < 0 - brakeing) is constructed based on the values
    ...
    def _cog_turning_angle(self):
        # The turning angle of the center of gravity. See the angle "beta" in the reference text.
    ...
    def _update_speed(self):
        # update speed assuming locally linear behaviour of acceleration and drag
        new_speed = (
            self._speed
    ...
    def update_car(self):
        # the order of the updates here is important, it should be simul time -> speed -> heading -> velocity -> position -> acceleration -> sys timestamp
